# train_env_encoder.py
# 분리된 env 스트림 전용 LCNN-SE 학습
#
# 목적: "이 env 스트림이 합성/가짜인가?"
# 레이블:
#   bonafide_bonafide → 0 (REAL env)
#   spoof_bonafide    → 0 (REAL env, speech만 합성)
#   bonafide_spoof    → 1 (FAKE env)
#   spoof_spoof       → 1 (FAKE env)
#
# 전제: extract_stream_cache.py 실행 완료
# Usage: python3 train_env_encoder.py
# 출력: pretrained_models/env_encoder.pt
...
SAVE_PATH     = "pretrained_models/env_encoder.pt"
...
# env가 합성인 compound label → 1 (SPOOF)
ENV_LABEL = {
    "original":          0,   # 원본 오디오 → env REAL
    "bonafide_bonafide": 0,
    "spoof_bonafide":    0,   # speech만 합성 → env는 REAL
    "bonafide_spoof":    1,   # env 합성
    "spoof_spoof":       1,   # env 합성
}
...
def load_cache(cache_path):
    ...
    lfccs  = data["env_lfcc"]                                # env 스트림
    clbls  = data["compound_labels"]
    labels = np.array([ENV_LABEL[l] for l in clbls], dtype=np.float32)
    return lfccs, labels
...
def train():
    print("=" * 55)
    print("Env Encoder 학습 (분리된 env 스트림 전용)")
    print("=" * 55)
